...
EOF
  ...
  for t in \
	...
	# Instala dependência dialog para alguns scripts
	  ...
	  apt-get install -y dialog || log "WARNING" "Falha ao instalar dialog"
...
EOF
	...
	# Função para parar serviços existentes antes de reinstalar
stop_existing_services() {
  log "INFO" "Verificando e parando serviços existentes..."

  local services=("fazai" "fazai-gemma-worker" "fazai-docler" "fazai-qdrant")

  for service in "${services[@]}"; do
    if systemctl is-active --quiet "$service" 2>/dev/null; then
      log "INFO" "Parando serviço $service..."
      systemctl stop "$service" 2>/dev/null || true
    fi
    if systemctl is-enabled --quiet "$service" 2>/dev/null; then
      log "INFO" "Desabilitando serviço $service..."
      systemctl disable "$service" 2>/dev/null || true
    fi
  done
...
configure_systemd() {
	  ...
	  # Para serviços existentes antes de reinstalar
	  stop_existing_services
...
EOF
	  ...
	  # Recarrega antes de habilitar novos serviços
	  systemctl daemon-reload
